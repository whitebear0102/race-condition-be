...
### Giai đoạn 3: Triển khai Kafka Consumer & Đồng bộ Database MySQL

- Khởi tạo `PrismaService` để quản lý kết nối và tương tác với cơ sở dữ liệu MySQL.
- Cấu hình ứng dụng NestJS chạy song song hai chế độ: HTTP Server (tiếp nhận API) và Microservice (lắng nghe Message Queue).
- Xây dựng `OrderConsumer` sử dụng decorator `@EventPattern('order_created')` để liên tục "hứng" các thông báo đơn hàng từ topic của Kafka.
- Áp dụng **Database Transaction** (`prisma.$transaction`) để đảm bảo tính nguyên tử (ACID): Thực hiện đồng thời việc tạo bản ghi `Order` mới và trừ số lượng tồn kho thực tế trong bảng `Product`. Nếu một trong hai thao tác thất bại, hệ thống sẽ tự động rollback.
- **Tối ưu Kiến trúc (Backpressure):** Tách biệt hoàn toàn luồng chốt đơn siêu tốc trên RAM (Redis) và luồng ghi dữ liệu bền vững trên ổ đĩa (MySQL). Kafka đóng vai trò "bộ đệm" hoàn hảo, giúp MySQL không bị quá tải.
- **Kết quả:** Hoàn thiện vòng đời của một đơn hàng Flash Sale. Dữ liệu của 5 người mua thành công đã được lưu trữ an toàn, vĩnh viễn vào cơ sở dữ liệu nền tảng.

### Giai đoạn 4: Xử lý Lũy đẳng (Idempotency) & Hoàn thiện Business Logic

- **Tái cấu trúc Database:** Gỡ bỏ ràng buộc Unique cứng nhắc `@@unique([productId, userId])` trên bảng `Order`. Bổ sung trường `idempotencyKey` (UUID) làm khóa Unique mới để phân biệt rạch ròi giữa "Chống lặp kỹ thuật" và "Logic nghiệp vụ".
- **Cập nhật Producer:** Sinh mã `uuid` duy nhất tại thời điểm người dùng click "Mua hàng" và đính kèm vào payload gửi sang Kafka.
- **Xây dựng Consumer 2 lớp kiểm tra (Two-layer Check):**
  - _Lớp 1 - Network Idempotency:_ Kiểm tra `idempotencyKey` trong DB. Nếu đã tồn tại -> Đây là gói tin bị gửi trùng do lỗi mạng (Retry) -> Bỏ qua và báo Kafka Commit Offset.
  - _Lớp 2 - Business Rule:_ Kiểm tra trạng thái đơn hàng. Nếu User đang có đơn `PENDING` hoặc `CONFIRMED` -> Chặn không cho mua thêm. Nhưng nếu đơn cũ là `FAILED` (đã hoàn kho) -> Vẫn cho phép User tạo đơn hàng mới.
- **Bắt lỗi & Phục hồi (Disaster Recovery):** Chủ động `throw error` đối với các lỗi hệ thống (như mất kết nối DB) để Kafka từ chối Commit Offset và tự động thực hiện quy trình Retry gửi lại tin nhắn khi hệ thống sống lại.
- **Kết quả:** Hệ thống đạt chuẩn Production-ready. Hoàn toàn miễn nhiễm với lỗi người dùng spam click, lỗi mạng chập chờn, hay server Consumer sập nguồn giữa chừng, đồng thời giữ được trải nghiệm người dùng (UX) linh hoạt.
